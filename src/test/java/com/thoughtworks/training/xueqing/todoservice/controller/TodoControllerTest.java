...
public class TodoControllerTest {
    ...
    private User userFixture = new User(1, "xue", "xue", ImmutableList.of());
    ...
    public void shouldReturnListWithSecurity()throws Exception{
        SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken("xue", null, Collections.emptyList()));
        when(userService.findByName("xue")).thenReturn(userFixture);
        when(todoRepository.findAllByUserIdEquals(1))
                .thenReturn(Collections.singletonList(new Todo(1, "todo 1", false,false,new Date(),1,ImmutableList.of())));
